...
const TimetableExportModal = dynamic(() => import('@/components/timetable/TimetableExportModal'), { ssr: false });
...
export default function TimetablePage() {
  ...
  return (
    <div className="content-pad animate-fade-in" style={{ padding: 24 }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
        <div>
          <div style={{ fontFamily: 'var(--font-head)', fontSize: 18, fontWeight: 700, display: 'flex', alignItems: 'center', gap: 8 }}>
            <Calendar size={20} color="var(--accent)" /> Weekly Timetable
          </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="btn btn-sm" onClick={fetchSlots} style={{ gap: 6 }}>
            <RefreshCw size={13} /> Refresh
          </button>
          <button className="btn btn-primary btn-sm" onClick={() => setShowExport(true)} style={{ gap: 6 }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12, marginBottom: 16, padding: '10px 14px', background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 10 }}>
        {[
          ...
          <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: 'var(--text3)' }}>
            {bar ? <div style={{ width: 4, height: 16, borderRadius: 2, background: color }} />
                 : <div style={{ width: 8, height: 8, borderRadius: '50%', background: color }} />}
            {label}
          </div>
        ))}
        <div style={{ marginLeft: 'auto', fontSize: 11, color: 'var(--text3)', fontStyle: 'italic' }}>
          ⏳ Mark done only available after slot ends
        </div>
      </div>

      {loading ? (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, color: 'var(--text3)', padding: '40px 0', justifyContent: 'center' }}>
          <Loader2 size={18} style={{ animation: 'spin 1s linear infinite' }} /> Building your timetable…
        </div>
      ) : slots.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 56, color: 'var(--text3)' }}>
          ...
          <div style={{ fontSize: 13 }}>Create your schedule first, then come back here for the full grid view.</div>
        </div>
      ) : (
      ...
      <AnimatePresence>
        {showExport && (
          <TimetableExportModal slots={slots} onClose={() => setShowExport(false)} />
        )}
      </AnimatePresence>
    </div>
  );
