...
// Icons: Font Awesome 6 via CDN (add to app/layout.tsx <head>)
...
// ─── Font Awesome icon map per tag ───────────────────────────────────────────
const TAG_ICONS: Record<string, { icon: string; color: string; bg: string }> = {
  MORNING_ROUTINE: { icon: 'fa-solid fa-sun',                color: '#F5A623', bg: 'rgba(245,166,35,0.12)'   },
  BREAKFAST:       { icon: 'fa-solid fa-mug-saucer',         color: '#FFD43B', bg: 'rgba(255,212,59,0.12)'   },
  LUNCH:           { icon: 'fa-solid fa-bowl-food',          color: '#74C0FC', bg: 'rgba(116,192,252,0.12)'  },
  DINNER:          { icon: 'fa-solid fa-utensils',           color: '#FF8787', bg: 'rgba(255,135,135,0.12)'  },
  EXERCISE:        { icon: 'fa-solid fa-dumbbell',           color: '#69DB7C', bg: 'rgba(105,219,124,0.12)'  },
  STUDY:           { icon: 'fa-solid fa-book-open',          color: '#74C0FC', bg: 'rgba(116,192,252,0.12)'  },
  WORK:            { icon: 'fa-solid fa-briefcase',          color: '#748FFC', bg: 'rgba(116,143,252,0.12)'  },
  BREAK:           { icon: 'fa-solid fa-mug-hot',            color: '#FFA94D', bg: 'rgba(255,169,77,0.12)'   },
  SLEEP:           { icon: 'fa-solid fa-moon',               color: '#845EF7', bg: 'rgba(132,94,247,0.12)'   },
  DEEP_WORK:       { icon: 'fa-solid fa-brain',              color: '#FF6B6B', bg: 'rgba(255,107,107,0.12)'  },
  MEETING:         { icon: 'fa-solid fa-users',              color: '#4DABF7', bg: 'rgba(77,171,247,0.12)'   },
  MEDITATION:      { icon: 'fa-solid fa-spa',                color: '#63E6BE', bg: 'rgba(99,230,190,0.12)'   },
  COMMUTE:         { icon: 'fa-solid fa-car',                color: '#A9E34B', bg: 'rgba(169,227,75,0.12)'   },
  PERSONAL_CARE:   { icon: 'fa-solid fa-shower',             color: '#74C0FC', bg: 'rgba(116,192,252,0.12)'  },
  READING:         { icon: 'fa-solid fa-book',               color: '#FFC078', bg: 'rgba(255,192,120,0.12)'  },
  CUSTOM:          { icon: 'fa-solid fa-bookmark',           color: '#868E96', bg: 'rgba(134,142,150,0.12)'  },
};
const DEFAULT_ICON = { icon: 'fa-solid fa-circle-dot', color: '#868E96', bg: 'rgba(134,142,150,0.12)' };
function getTagIcon(tag: string) { return TAG_ICONS[tag] ?? DEFAULT_ICON; }

// ─── SlotIcon component ───────────────────────────────────────────────────────
function SlotIcon({ tag, size = 14 }: { tag: string; size?: number }) {
  const { icon, color, bg } = getTagIcon(tag);
  return (
    <div style={{
      ...
      <i className={icon} style={{ fontSize: size, color }} />
    </div>
  );
}
...
interface BuilderSlot {
  id: string;
  _isNew?: boolean;
  emoji: string;         // kept for DB compat; stores FA class string now
  name: string;
  startTime: string;
  endTime: string;
  tag: SlotTag;
...
function makeSlot(name: string, start: string, end: string, tag: SlotTag): BuilderSlot {
  return {
    id: uuid(), _isNew: true,
    emoji: getTagIcon(tag).icon,   // store FA class string in emoji field
    name, startTime: start, endTime: end, tag,
    checklistOn: false, checklist: [], strict: false,
    strictMode: 'WARN', repeatDays: [1, 2, 3, 4, 5, 6, 7],
...
function redistributeTimes(slots: BuilderSlot[]): BuilderSlot[] {
  ...
  let cursor = toMins(slots[0].startTime);
...
export default function ScheduleBuilder({ onSave }: Props) {
  ...
  const [dbSlotIds, setDbSlotIds] = useState<Set<string>>(new Set());
      ...
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          const loaded = data.map((s: any): BuilderSlot => ({
            id: s.id, _isNew: false,
            emoji: s.emoji || getTagIcon(s.tag).icon,
            name: s.title,
            ...
            isAutoMark: s.isAutoMark || false, emailAlert: s.emailAlert || false,
  ...
  const handleFieldChange = useCallback((id: string, field: keyof BuilderSlot, val: any) => {
    setSlots(prev => prev.map(s => {
      if (s.id !== id) return s;
      // When tag changes, auto-update the stored icon class too
      if (field === 'tag') {
        return { ...s, tag: val as SlotTag, emoji: getTagIcon(val as string).icon };
      }
      return { ...s, [field]: val };
    }));
  }, []);
  ...
  return (
    <div>
          ...
          <motion.div
            ...
            <div>
              ...
              {shiftEvents.slice(0, 3).map((ev, i) => (
                <div key={i} style={{ opacity: 0.8 }}>{ev.slotName}: {fmt(ev.oldStart)} → {fmt(ev.newStart)}</div>
              ))}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', borderRadius: 8, background: 'rgba(55,138,221,0.06)', border: '0.5px solid rgba(55,138,221,0.2)', marginBottom: 12, fontSize: 12, color: '#0C447C' }}>
        <i className="fa-solid fa-circle-info" style={{ fontSize: 13, color: '#378ADD' }} />
          ...
          <Reorder.Item key={slot.id} value={slot} style={{ marginBottom: 8, cursor: 'grab' }}>
            <motion.div layout style={{ background: 'var(--surface)', border: '0.5px solid var(--border)', borderRadius: 10, overflow: 'hidden' }} whileHover={{ borderColor: 'var(--border2)' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px' }}>
                ...
                {/* Icon — auto-derived from tag, updates when category changes */}
                <SlotIcon tag={slot.tag} size={14} />
                ...
                <input
                  ...
                  style={{ flex: 1, fontSize: 13, fontWeight: 500, background: 'transparent', border: 'none', outline: 'none', cursor: 'text', color: 'var(--text)', fontFamily: 'var(--font-body)', minWidth: 0 }}
                />
                ...
                {/* Duration badge */}
                <div style={{
                  fontSize: 11, color: 'var(--text3)', flexShrink: 0, minWidth: 36,
                  background: 'var(--surface2)', borderRadius: 6, padding: '2px 6px',
                  border: '0.5px solid var(--border)', textAlign: 'center',
                }}>
                  {(() => { const d = duration(slot.startTime, slot.endTime); return d >= 60 ? `${Math.floor(d / 60)}h${d % 60 ? ` ${d % 60}m` : ''}` : `${d}m`; })()}
                </div>
                ...
                {/* Delete */}
                <button
                  onClick={e => { e.stopPropagation(); if (confirm('Delete this slot?')) deleteSlot(slot.id); }}
                  ...
                  <motion.div
                    ...
                    <div style={{ borderTop: '0.5px solid var(--border)', padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                          {(Object.keys(TAG_CONFIG) as SlotTag[]).map(t => {
                            const ti = getTagIcon(t);
                            const active = slot.tag === t;
                            return (
                              <button
                                key={t}
                                ...
                                style={{
                                  ...
                                  background: active ? ti.bg : 'var(--surface2)',
                                  color: active ? ti.color : 'var(--text3)',
                                  borderColor: active ? ti.color : 'var(--border)',
                                  display: 'flex', alignItems: 'center', gap: 5,
                                  transition: 'all 0.15s',
                                }}>
                                <i className={ti.icon} style={{ fontSize: 11, color: active ? ti.color : 'var(--text3)' }} />
                                {TAG_CONFIG[t].label}
                              </button>
                            );
                          })}
                        ...
                        <div style={{ display: 'flex', gap: 4 }}>
                          {['Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa', 'Su'].map((d, i) => {
                            ...
                            return (
                              <button
                                key={d}
                                onClick={() => handleFieldChange(slot.id, 'repeatDays', sel ? slot.repeatDays.filter(x => x !== dayNum) : [...slot.repeatDays, dayNum].sort())}
                                style={{ width: 32, height: 32, borderRadius: '50%', fontSize: 10, fontWeight: 600, border: '0.5px solid var(--border2)', cursor: 'pointer', background: sel ? 'var(--accent)' : 'var(--surface)', color: sel ? 'white' : 'var(--text3)' }}>
                                {d}
                              </button>
                            );
                          })}
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 14 }}>
                        {[
                          { label: 'Strict',        icon: 'fa-solid fa-lock',        field: 'strict',       val: slot.strict       },
                          { label: 'Auto-complete', icon: 'fa-solid fa-circle-check', field: 'isAutoMark',  val: slot.isAutoMark   },
                          { label: 'Email alert',   icon: 'fa-solid fa-envelope',    field: 'emailAlert',   val: slot.emailAlert   },
                          { label: 'Food required', icon: 'fa-solid fa-wheat-awn',   field: 'foodRequired', val: slot.foodRequired },
                        ].map(({ label, icon, field, val }) => (
                          <label key={label} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer' }}>
                            <input type="checkbox" checked={!!val} onChange={e => handleFieldChange(slot.id, field as any, e.target.checked)} style={{ accentColor: 'var(--accent)' }} />
                            <i className={icon} style={{ fontSize: 11, color: 'var(--text3)' }} />
                            {label}
                          </label>
                        ))}
                        {slot.strict && (
                      ...
                      <div>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 500 }}>
                            {slot.checklistOn
                              ? <i className="fa-solid fa-square-check" style={{ fontSize: 14, color: 'var(--accent)' }} />
                              : <i className="fa-regular fa-square" style={{ fontSize: 14, color: 'var(--text3)' }} />
                            }
                            Checklist
                          </div>
                            ...
                            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} style={{ overflow: 'hidden' }}>
                              {slot.checklist.map(item => (
                                <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 0', fontSize: 12 }}>
                                  <i className="fa-regular fa-circle" style={{ fontSize: 8, color: 'var(--text3)', flexShrink: 0 }} />
                                  <span style={{ flex: 1, color: 'var(--text2)' }}>{item.label}</span>
                                  <span
                                    style={{ fontSize: 10, padding: '1px 5px', borderRadius: 20, border: '0.5px solid var(--border)', background: item.required ? 'rgba(226,75,74,0.08)' : 'var(--surface2)', color: item.required ? '#E24B4A' : 'var(--text3)', cursor: 'pointer' }}
                                    onClick={() => setSlots(prev => prev.map(s => s.id === slot.id ? { ...s, checklist: s.checklist.map(c => c.id === item.id ? { ...c, required: !c.required } : c) } : s))}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <button className="btn btn-primary" style={{ flex: 1, justifyContent: 'center', display: 'flex', alignItems: 'center', gap: 7 }} onClick={handleSave} disabled={saving}>
          {saving
            ? <><Loader2 size={13} style={{ animation: 'spin 1s linear infinite' }} /> Saving…</>
            : <><i className="fa-solid fa-floppy-disk" style={{ fontSize: 13 }} /> Save schedule</>
          }
        </button>
        <button className="btn btn-sm" onClick={() => window.location.reload()}>
          <i className="fa-solid fa-rotate-left" style={{ fontSize: 12 }} /> Reset
        </button>
      </div>
    </div>
  );
...
function ChecklistAdder({ onAdd }: { onAdd: (item: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
      ...
      />
      <button className="btn btn-sm" onClick={() => { if (val.trim()) { onAdd(val); setVal(''); } }}>
        <i className="fa-solid fa-plus" style={{ fontSize: 11 }} /> Add
      </button>
    </div>
  );
}